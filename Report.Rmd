...
This exercise examines the Space Shuttle *Challenger*'s 1986 disaster using the previous safety record of the Space Shuttle Program. The probability of O-ring failures at various launch temperatures are modeled to make a prediction for the conditions on the morning of the disaster. It is found that the probability of a catastrophic failure is 34%, over three times higher than when using the methods of the original estimate. The Rogers Commission, tasked with investigating the accident, noted that this underestimate occurred due to omitting launch data where no O-ring damage was found.$^{[2]}$ The original estimate procedure was used by [Morton Thiokol](https://en.wikipedia.org/wiki/Thiokol), a contractor to NASA and producer of the O-ring involved component that failed. 
...
Because one of the key features of this dataset is the low number of observations, at 23 total, it was assumed that this would be a primary contributor to error in a model's predictions. From this, a procedure was designed where each iteration would:
...
Similar to before, the probability of an individual O-ring failing at the launch temperature of the *Challenger* was estimated via parametric bootstrap to be `r round(100*mean_challenger, 1)`%. The bootstrapped 95% confidence interval was `r round(100*ci_challenger[1], 1)`% and `r round(100*ci_challenger[2], 1)`%. At `r temp_in_sample`°F, the same method estimated the probability at `r round(100*mean_in_sample, 1)`% with a 95% confidence interval of `r round(100*ci_in_sample[1], 1)`% and `r round(100*ci_in_sample[2], 1)`%. Note that in both of these intervals, bounds extend beyond the limit of 0% and 100% similar to the Wald interval. Although bootstrapping has become popular in recent decades with the rise of computation power and simple application to many contexts, certain issues can still be encountered depending on the use case. 
...
grid.arrange(plt_c, plt_is, nrow = 1,
             top = textGrob(
               label = "Figure 6. Bootstrapped Probabilities of Failure"),
             bottom = textGrob(label = expression(pi)),
             left = textGrob(label = "Count", rot = 90))
```

**Figure 6** shows the bootstrapped probabilities of failure for a single O-ring at 36°F and 70°F. The confidence intervals determined from the distribution are also shown in orange for each temperature. The "parametric" nature of the bootstrap can be observed as both resemble a normal distribution, although the boundaries at 0% and 100% add a slight skew. However, these are both much more similar to normal distribution than what is shown in **Figure 5**, suggesting the bootstrap is more reliable than the Wald intervals even though both have similar boundary problems. 
...
linear_model %>%
  ...
  labs(
    title = "Figure 7. Predicted Probabilities vs. Standardized Residuals",
    x = "Predicted Values",
    y = "Standardized Residuals")
```

Shown in **Figure 7**, the standardized residuals are not constant over the range of the output variable in the model. This does not meet the homoscedasticity assumption, because the output variable is itself a function of the input variables. This result was expected, however, as it can be shown that the variance in a random variable from a Binomial distribution is a function of itself, or $Var(\pi) \propto \pi*(1-\pi)$. It can be further seen in **Figure 7** that there is a general upward trend as $\pi \to 0.5$, which follows from the variance relationship. Given that the homoscedasticity assumption is not met, there is further evidence of preferring the logistic model to the linear regression.
...
where $\hat{\pi}$ represents the probability of at least one O-ring failure per launch in the binary model and the probability of an individual O-ring failing in the full model.
...
In the binary model, the intercept $\hat{\beta_0}$ had the value `r beta_0_binary` while the coefficient of temperature $\hat{\beta_1}$ had the value `r beta_1_binary`. The coefficient of temperature can be interpreted as increasing the odds of any O-ring failure by `r exp(-1*beta_1_binary)` with every 1°F decrease in temperature and all else constant. Likewise, in the full model the intercept $\hat{\beta_0}$ had the value `r beta_0_binom` while the coefficient of temperature $\hat{\beta_1}$ had the value `r beta_1_binom`. In this case, for every 1°F decrease in temperature with all else constant the odds of a single O-ring failing would be increased by `r exp(-1*beta_1_binom)`.
...
The original estimate by Morton Thiokol omitted all launch data when no O-ring damage was observed under the assumption that it did not have anything to offer to an analysis of O-ring damage. This reduced the number of data points from 23 to only six. More importantly, such an action makes it fundamentally impossible to understand why some O-rings fail while others don't when only considering failed O-rings and not the entire group. If this was corrected, the relationship between temperature and O-ring damage could have been learned with the available data. 

Dalal et al. further went on to estimate the probability of secondary O-ring failure with additional data. It was found that given failure in a primary O-ring, a secondary O-ring had a probability of failure equal to $p = 0.286 * 0.292 \approx 8.4$%. This comes from the probability of erosion followed by the probability of blowby in a secondary O-ring, similar to what could occur in a primary O-ring. Taking this, the full model's estimate of a catastrophic failure for the *Challenger* launch would be $0.084 * 4 \approx 34$% given that only one secondary O-ring needed to fail out of the four behind the failed primary O-rings. Certain engineers within Morton Thiokol who had a similar belief and suggested this, but had their opinions dismissed. Using the model developed with the company's methods of dropping non O-ring damaged launches, the probability of catastrophe would only be estimated at `r round(100*0.084*6*exp(beta_0_morton + beta_1_morton*temp_challenger)/(1 + exp(beta_0_morton + beta_1_morton*temp_challenger)), 1)`%. Therefore, it would have been possible to predict that the launch temperature of the *Challenger* disaster was very likely to end in a catastrophic failure. 

Had the launch been postponed only a few hours to reach that day's noon temperature of 66°F, the probability of an individual O-ring failure would have dropped to `r round(100*exp(beta_0_binom + beta_1_binom*66)/(1 + exp(beta_0_binom + beta_1_binom*66)), 1)`% and the expected number of O-ring failures for the launch to ~`r round(6*exp(beta_0_binom + beta_1_binom*66)/(1 + exp(beta_0_binom + beta_1_binom*66)), 1)`. At a value less than one, it would be expected that no primary O-rings failed. In this scenario, the probability of a catastrophe would only have been $0.084*0.4\approx3.4$%. Even this remains high, and had this flaw been focused on earlier then work could have proceeded to make the launch system safer and more reliable. It can be concluded that the catastrophe was avoidable had the program appropriately analyzed data by factoring in both the successful and failed launches, and that the O-ring flaw was inherent up to this point in Shuttle design. 
