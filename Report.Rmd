...
With the assumption of independence, two different uses of the Binomial distribution will be investigated. The first will model *n* = 6 IID O-rings on each flight, each with an identical probability *p* of experiencing distress, as a single Bernoulli trial. The series of Shuttle launch observations, each with their own Bernoulli trials, will then make up a Bernoulli process. This will be referred to as the "full" outcome throughout the rest of this report. The second will investigate only the probability of a launch experiencing *any* O-ring distress by use of the binary `distress` variable created during the EDA. This will model the reduced *n* = 1 Bernoulli trial of whether at least one O-ring was distressed on a launch, with a single probability *p* of this occurring. The series of launches will likewise make up a Bernoulli process, and be referred to as the "binary" outcome throughout this report.

An advantage of the former is that it uses all available information in the dataset, potentially estimating a more precise relationship between O-ring failure and environmental conditions. It also can provide a more useful prediction for the outcome of the *Challenger* disaster, since any design-flaw failure in a secondary O-ring when the primary is distressed would cause a catastrophe. Having an estimate of a specific number of primary O-rings which fail would then be proportional to the probability of the catastrophe. The use of the binary `distress` outcome, however, only estimates if at least one O-ring will fail without regard to how many. Still, because of its simpler nature it can potentially provide a more accurate estimate when the number of observations is so low. Therefore, the full model will be used to provide a primary estimate of the disaster's probability, with the binary model used as a spot-check against its results.

There are two explanatory variables of interest, `Temp` and `Pressure`. To test whether either of these are significant, a series of three models will be created.
$$PUT FIRST MODEL HERE$$
The first, shown above, will only include an intercept and serve as the null hypothesis, where neither `Temp` nor `Pressure` have a relationship with O-ring distress. This equivalent to $\beta_{Temp} = \beta_{Pressure} = 0$.
$$SECOND$$
The second will include `Temp` as the sole independent variable. This is motivated by how Morton Thiokol was originally aware of O-ring condition deteriorating at lower temperatures using the same dataset. 
$$Third$$
Finally, `Pressure` will be included along with `Temp` in the third model to see if both environmental conditions can also be related to O-ring distress. As common with a Binomial distribution, a logit link function will be used to join the dependent and independent variables in all three models. 

```{r}
# Null hypothesis, no relationship between env and o-rings
binary_null <- glm(formula = distress ~ Temp, data = d, 
                   family = binomial(link = logit))

# First alternative hypothesis, only temp has relationship
binary_temp <- glm(formula = distress ~ Temp + Pressure, data = d, 
                   family = binomial(link = logit))
...
# Null hypothesis, no relationship between env and o-rings
binom_null <- glm(formula = O.ring.fraction ~ Temp, data = d, 
                  family = binomial(link = logit))

# First alternative hypothesis, only temp has relationship
binom_temp <- glm(formula = O.ring.fraction ~ Temp + Pressure, data = d, 
                  family = binomial(link = logit))
...
The same set of three models will be used for both the full and binary outcome variables, shown above. A likelihood ratio test (LRT) will be used to determine whether a model associated with an alternative hypothesis is a significant improvement over the null hypothesis model. If the first alternative hypothesis is significant, i.e. involving only `Temp`, then the second alternative hypothesis will be assessed against it. 
