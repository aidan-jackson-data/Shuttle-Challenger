...
```{r}
# calculate Wald standard errors for model coefficients
binary_se <- sqrt(diag(vcov(binary_temp)))
binom_se <- sqrt(diag(vcov(binom_temp)))
```
...
# standard errors
binary_ses <- binary_prob$se.fit
binom_ses <- binom_prob$se.fit

# 2.5th percentile
binary_lower_ci <- binary_pis - qnorm(p = 1 - alpha/2)*binary_prob$se
binom_lower_ci <- binom_pis - qnorm(p = 1 - alpha/2)*binom_prob$se
# 97.5th percentile
